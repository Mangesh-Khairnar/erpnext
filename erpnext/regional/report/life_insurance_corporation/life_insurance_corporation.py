...
def get_data(filters):
	...
	employee_insurance_record = frappe.get_all("Employee Insurance", 
									fields=["employee","policy_no","monthly_premium"],
									filters={
										"deduct_from_salary" : True
									})
	conditions = get_conditions(filters)

	entry = frappe.db.sql(""" select sal.employee, sal.employee_name, sal.department, ded.salary_component, ded.amount 
	...
	for d in entry:
		...
		policy_number_list=[str(e['policy_no']) for e in employee_record]
		monthly_premium_list=[str(e['monthly_premium']) for e in employee_record]
		employee["policy_number"] = "\n".join(policy_number_list)
		employee["premium"] =  "\n".join(monthly_premium_list)
		total += d.amount
